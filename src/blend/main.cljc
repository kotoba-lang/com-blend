(ns blend.main
  "Kotodama WASM entrypoint for the Blend clean-room actor (L4) — Clojure port.
  ...
  facts (`blend.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(def entity-specs
  [{:entity "Account"     :plural "accounts"     :id-prefix "blend_acc"
    :fields [:holderId :type :balance :currency :status] :required [:type :balance]
    :coerce {:balance :float}     :refs {}}
   {:entity "Loan"        :plural "loans"        :id-prefix "blend_loa"
...
(defn emit-facts
  "EAVT facts for one record: {\"blend.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "blend-compat" :tier tier :entities entities} 200])
